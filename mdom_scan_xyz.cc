...
// preparing file for results.
	...
	datafile << std::fixed << std::setprecision(3) << gtheta << "\t" << gphi << "\t" << gposX << "\t" << gposY << "\t" << gposZ << "\t" << gwavelen << "\t"<< gsimevents<< "\t" << BeamRad;	
	datafile.close();
...
int main(int argc,char *argv[])
{
	...
	const char* progname = "mdom_scan_xyz";
	int nerrors;
	int exitcode=0;
	...
	events->ival[0] = 0;
	gunfile->filename[0] = "mdom_scan_xyz.gps";
	...
	outputfile->filename[0] = "../output/mdom_testoutput_scan_xyz.txt";
    ...
    if (help->count > 0)
	{
        printf("\nGEANT4 simulation of the mDOM: local acceptance scan\n");
        printf("\nUsage: %s", progname);
